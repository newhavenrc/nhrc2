...
def read_snow_related_issues(readfile=False, writejson=False):
    """
    ...
    issue_id = '8516'
    issurl = 'https://seeclickfix.com/api/v2/issues?request_types='+issue_id+'&per_page=10000'
    json_data = json.load(urllib2.urlopen(issurl))

    if readfile:
        json_data = json.load(open(nhrc2dir+'data/scf_data_iss'+issue_id+'.json', 'r'))
    else:
        json_data = json.load(urllib2.urlopen(issurl))

    if writejson:
        json.dump(json_data, open(nhrc2dir+'data/scf_data_iss'+issue_id+'.json', 'w'))

    scf_iss_df = pd.DataFrame(jq(record_rule).transform(json_data))
    scf_iss_df['issue_id'] = issue_id
    scf_iss_df['category'] = 'SNOW RELATED'
